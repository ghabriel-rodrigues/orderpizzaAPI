...
//required token, email, password and orderData: shoppingCartID, cvc, flag['tok_mastercard','tok_visa'], expDate and cardNumber
//optional: none
handlers._orders.post = async function (data, callback) {
  ...
  let shoppingCartId = typeof (data.payload.shoppingCartId) == 'string' &&
    data.payload.shoppingCartId.trim().length == 20 ?
        ...
        _data.read('users', userValidated.email, function (err, userData) {
          if (!err && userData) {
            ...
            if (hashedPassword == userData.hashedPassword) {
              ...
              let hashedExpirationDate = helpers.hash(orderValidated.expirationDate);
              ...
              _data.create('orders', orderId, orderObject, function (err) {
                if (!err) {
                  console.log('order created, creating the purchase', orderObject);
                  helpers.chargingStripe(orderObject.amount, 'usd', orderObject.flag, function (err) {
              ...
              _data.read('orders', orderId, function (err, orderData) {
                if (!err && orderData) {
                  if (orderData.expires > Date.now()) {
                    ...
                    orderData.flag = orderValidated.flag;
                    orderData.amount = orderValidated.amount;
...
handlers._menuItems.get = function (data, callback) {
  ...
  if (id) {
    ...
    let email = typeof (data.headers.email) == 'string' ? data.headers.email : false;
