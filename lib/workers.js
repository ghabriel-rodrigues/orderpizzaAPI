...
workers.validateCheckData = function (originalCheckData) {
  ...
  let emailRegex = /[^@ \t\r\n]+@[^@ \t\r\n]+\.[^@ \t\r\n]+/;
  originalCheckData.userEmail = typeof (originalCheckData.userEmail) == 'string' &&
    originalCheckData.userEmail.trim().length > 3 &&
    emailRegex.test(originalCheckData.userEmail.trim()) ?
    originalCheckData.userEmail.trim() : false
...
workers.alertUserToStatusChanged = function (newCheckData) {
  ...
  //TODO:advertise the user about the shop?
}
...
workers.log = function (originalCheckData, checkoutOutcome, state, alertWarranted, timeOfCheck) {
  ...
  //determine the nme of the log file
