...
helpers.validateOrder = function (data) {
  ...
  let flag = flagAcceptable.indexOf(data.payload.flag) > -1 &&
    data.payload.flag.trim().length > 0 ?
    data.payload.flag : false;
  ...
  let shoppingCartId = typeof ((data.payload.shoppingCartId) == 'object') &&
    data.payload.shoppingCartId instanceof Array ? data.payload.shoppingCartId : [];
...
helpers.calculateAmount = function (shoppingCartId) {
  let amount = 0;

  handlers._data.read('shoppingCart', shoppingCartId, function (err, shoppingCartData) {
    if (!err && shoppingCartData) {
      shoppingCartData.menuItemsOrderedIds.reduce((total, itemId) => {
        handlers._data.read('menuItems', itemId, function (err, menuItemData) {
          if (!err && menuItemData) {
            amount = total + menuItemData.price;
          }
        });
      });
    }
